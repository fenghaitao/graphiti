...
async def generate_summary_description(llm_client: LLMClient, summary: str) -> str:
    ...
    # Log if description is empty
    if not description or description.strip() == '':
        logger.warning(f"Generated empty description for summary: {summary[:100]}...")
        logger.warning(f"LLM response type: {type(llm_response)}, value: {llm_response}")
