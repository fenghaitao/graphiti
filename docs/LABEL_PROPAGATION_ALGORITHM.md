...
After detecting communities, we generate hierarchical summaries by combining entity summaries in a tournament-style approach.

## Degree-Based Ordering

Entities are sorted by degree (ascending) before summarization:

```python
# In client.py
entities_sorted = sorted(entities, key=lambda e: e.degree)
```

**Why ascending order**:
- Peripheral nodes (low degree) are summarized first
- Hub nodes (high degree) are summarized last
- Creates a natural hierarchy: details → themes
...
**Example with 4 nodes** (sorted by degree):
```
[A(deg=1), B(deg=2), C(deg=3), D(deg=4)]
...
The current approach doesn't follow actual graph edges - it only uses degree as a proxy for centrality. True graph-aware hierarchical summarization would:

1. Start from peripheral nodes (degree 1)
2. Follow edges inward toward the center
3. Ensure paired nodes are actually connected

This enhancement is deferred for future work.

## LLM Integration

Each pair of summaries is combined using an LLM:
...
The final summary is then used to generate a community name/description.
